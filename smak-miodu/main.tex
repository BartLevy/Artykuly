...
Stwierdzona różnica typowości smaku między miodami dopuszczonymi
...
Wszystkie próbki zostały przebadane w certyfikowanych laboratoriach, 
a uzyskane wyniki spełniały wymagania w zakresie analizy pyłkowej (zgodność z deklaracją producenta), 
zawartości antybiotyków, profilowania metodą NMR, przewodności elektrycznej oraz składu cukrów. 
Zestaw obejmował dwa miody akacjowe (z robinii akacjowej), cztery miody wielokwiatowe oraz jeden miód lipowy. 
W przypadku czterech próbek odnotowano zastrzeżenia dotyczące nienaturalnego charakteru smaku, 
których szczegółowy opis uzupełniono na podstawie dostępnych danych sensorycznych. 
Wszystkie próbki przechowywano w kontrolowanych warunkach (ustalona temperatura, ograniczony czas przechowywania, 
ochrona przed światłem), co miało zapewnić stabilność ich właściwości do momentu oceny.

Próbki przechowywano w kontrolowanych warunkach (temperatura, czas, ochrona przed światłem), 
zapewniając ich stabilność do momentu oceny.
...
Właściwe ograniczenie leży gdzie indziej: status dopuszczenia do obrotu opisuje
\emph{całą partię}, a partii było siedem. Przy takiej liczbie jednostek
klastrowych nie da się oddzielić „efektu statusu" od „efektu właściwości tego
konkretnego miodu", model przypisze zmiennej statusu każdą różnicę między
partiami, niezależnie od jej prawdziwego źródła.

Dlatego najmocniejszy wniosek nie wynika z jednego testu, tylko z tego, że kilka
różnych analiz mówi to samo. Gdy zostały porównane miody w obrębie tego samego rodzaju
(co usuwa wpływ gatunku), dla miodów akacjowych (robinia fałszywa) różnicy nie było, a dla miodów
wielokwiatowych całą różnicę „robiła" jedna, wyjątkowo wysoko oceniona próbka.
Sprawdzona została też moc badania (czyli jego zdolność do wykrycia efektu) i okazała
się ona dalece za niska, by cokolwiek pewnego stwierdzić \cite{Cohen1988}. 
Wszystko to razem wskazuje, że pozorna istotność bierze
...
istnieje, tylko czy na podstawie smaku da się rozstrzygnąć status (dopuszczenia do obrotu) miodu. 
