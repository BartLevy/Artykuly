...
Każda faza produkuje dane wejściowe dla fazy następnej, a wynik całości, czyli ocena zgodności podmiotu z aktualnym stanem regulacyjnym z punktu widzenia ITC,  jest wynikiem wszystkich odpowiedzi na każdym etapie.
...
Faza pierwsza składa się z dwóch kroków wykonywanych wyłącznie raz, przed przystąpieniem do analizy jakiegokolwiek obiektu i~dotyczy globalnie całe orgnanizacji.
...
Wynikiem operacji indetyfikacji będzie lista obiektów typu \texttt{SYSTEM\_INFORMATYCZNY}, która będzie dołączona do grafu ITC.
...
(np.\ DORA, NIS2, Rekomendacja D KNF)?
...
\item \textbf{Perspektywa użytkownika:} Czy jako użytkownik systemu masz pewności, czy fizycznie dane przechowywane są na serwerach kraju EOG?
\item \textbf{Perspektywa techniczna:} Czy kopie zapasowe lub dane produkcyjne systemu są składowane na zasobach poza kontrolą  dministracyjną organizacji?
\end{itemize}
...
\bigcup_{\tau \in \mathcal{T}}
  \mathcal{S}_{\tau} \cdot \mathbb{1}\bigl[\mathrm{type}(v) = \tau\bigr]
\;\cup\;
\bigcup_{k}
  \mathcal{S}_k \cdot \mathbb{1}\bigl[\Phi_k\bigl(\mathcal{A}(v)\bigr)\bigr]
\label{eq:sekcje}
\end{equation}
...
Jednym z najpoważniejszych i~najczęściej pomijanych problemów współczesnej praktyki compliance jest zjawisko \textit{deklaratywnego zatrzymania łańcucha} analizy poddostawcóœ: 
...
\textbf{niezależnie od liczby obsługiwanych systemów} lub jeśli obsługuje co najmniej trzy systemy podmiotu, niezależnie od ich poziomu krytyczności.
Spełnienie wyłącznie jednego warunku \textbf{jest wystarczające}.
...
\midrule
\texttt{BEZPIECZENSTWO\_TECHNICZNE}
...
\begin{itemize}
    \item \textbf{Linia ciągła} ($\rightarrow$) -- standardowa zależność usługowa; system korzysta z usług dostawcy na podstawie umowy;
    \item \textbf{Linia przerywana} ($\dashrightarrow$) -- krawędź odkryta przez trigger TIA; oznacza poddostawcę niewidocznego na etapie Fazy~2, ujawnionego dopiero w toku analizy kwestionariusza szczegółowego;
    \item \textbf{Etykieta \texttt{[KV]}} na krawędzi, dostawca kluczowy (\textit{KeyVendor}), spełniający co najmniej jeden warunek z definicji~\ref{def:dostawca-kluczowy}: obsługuje co najmniej jeden system KRYTYCZNY lub obsługuje co najmniej $n_{KV} = 3$ systemy podmiotu.
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Gdzie skupia się ryzyko?} Wierzchołki oznaczone $[\times]$, $[\times!]$ lub $[\times!!]$ wymagają natychmiastowej uwagi, niezależnie od ich głębokości w grafie.
    \item \textbf{Który dostawca jest kluczowy?} Krawędzie z etykietą \texttt{[KV]} wskazują podmioty, których awaria lub niezgodność zagraża stabilności całej organizacji.
    \item \textbf{Jak głęboko sięga łańcuch?} Liczba poziomów grafu ujawnia, czy organizacja jest świadoma pełnej ekspozycji na ryzyko poddostawców, co jest wymogiem art.~28 DORA i~art.~6 ust.~2 dyrektywy NIS2.
\end{enumerate}
