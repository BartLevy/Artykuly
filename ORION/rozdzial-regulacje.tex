...
\subsection{Zakres obowiązków wynikający z ustawy o~KSC z~2018 r.}
...
Podmioty kluczowe i ważne zobowiązane są do zgłaszania \textit{poważnych incydentów} do właściwego zespołu CSIRT lub organu nadzorczego. Za poważny
...
Dyrektywa NIS2 wprowadza mechanizm \textit{samoidentyfikacji}: to sam podmiot, a nie organ nadzorczy, ma obowiązek ustalić, czy mieści się w zakresie
...
podstawowym dokumentem obowiązującym banki była \textit{Rekomendacja D KNF}\autocite{knf_rekomendacja_d_2013}.
...
Rozporządzenie Parlamentu Europejskiego i~Rady (UE) 2022/2554, znane jako \textbf{DORA} (\textit{Digital Operational Resilience Act}), weszło w~życie 16~stycznia 2023~roku, a~jego stosowanie stało się obowiązkowe od 17~stycznia 2025~roku.
...
Czwarty filar stanowi najbardziej przełomowy element rozporządzenia. DORA nakłada bezpośrednie obowiązki na zewnętrznych dostawców ICT, w~tym hiperscalerów chmurowych takich jak Google Cloud, Microsoft Azure czy Amazon Web Services, obejmując ich nadzorem jako \textit{Critical ICT Third-Party Providers
...
o~prawie do audytu, lokalizacji przetwarzania danych, planach wyjścia (\textit{exit strategy}) oraz minimalnych okresach przejściowych przy zmianie dostawcy~\autocite{eiopa_dora}.
Korzystanie z~niezatwierdzonych narzędzi chmurowych (\textit{shadow IT}) stanowi naruszenie tego filaru, a~jednocześnie naruszenie zasady rozliczalności
z~art.~5 ust.~2 RODO.
...
Za naruszenie rozporządzenia Wiodący Organ Nadzorczy może nakładać na krytycznych dostawców ICT kary w~wysokości do 1\% średniego dziennego światowego obrotu za każdy dzień naruszenia, przez okres do sześciu miesięcy. Dla podmiotów finansowych nadzorowanych przez KNF sankcje określa prawo krajowe
wdrażające uprawnienia nadzorcze wynikające z~rozporządzenia~\autocite{knf_dora}.
