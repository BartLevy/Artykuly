...
oznaczane symbolem $d \in \{0;\ 0{,}4;\ 0{,}7;\ 1{,}0\}$.

\nopagebreak % Zapobiega łamaniu strony bezpośrednio przed tabelą, jeśli to konieczne
\renewcommand{\arraystretch}{1.6}
\begin{longtable}{p{1.5cm}p{2.5cm}p{4.5cm}p{4.5cm}}
\caption{Skala jakości dowodów w metodologii ORION} \label{tab:dowody} \\
\toprule
\textbf{Poziom} & \textbf{Symbol $d$} & \textbf{Definicja} & \textbf{Przykłady} \\
\midrule
\endhead % Koniec nagłówka dla pierwszej strony
...
Bardzo ważnym elementem planu ciągłości działania jest strategia komunikacji na czas kryzysu, który określa, kto, kiedy i~w jaki sposób informuje klientów, pracowników, kontrahentów oraz nadzór o wystąpieniu awarii i~przewidywanym czasie jej usunięcia. 
...
W metodologii ORION, plan ciągłości działania jest wymagany dla podmiotów określonych w KSC oraz DORA. Jego brak lub nieprawidłowe zdefiniowanie w organizacjach, gdzie jest on obowiązkowy wskazują binarnie na uchybienia w działaniu organizacji.
...
\section{Plan wyjścia z usługi (Exit plan)}

Plan wyjścia z usługi (ang. \textit{Exit plan})) to obowiązkowy element zarządzania dostawcami IT w banku, wymagany przez rozporządzenie DORA oraz ustawę KSC.
...
Według definicji DORA, plan musi wskazywać rzeczywistego dostawcę zastępczego lub opisywać scenariusz powrotu do infrastruktury własnej. Co istotne, umowa z dostawcą musi zawierać klauzulę zobowiązującą go do udostępnienia wszystkich danych w formacie umożliwiającym ich import do innego systemu
...
Ważna jest forma przygotowania dokumentacji. DORA, w art 28 ust. 4\autocite{dora2022} wymaga oddzielnego dokumentu, natomiast KSC wymaga realizuje cele exit planu poprzez klauzule umowne z dostawcą.

W metodologii ORION, exit plan jest wymagany dla podmiotów określonych w KSC oraz DORA. Podobnie jak plan ciągłości działania,  brak lub nieprawidłowo zdefiniowany exit plan, gdy jest obowiązkowy, wskazuje binarnie na uchybienia w działaniu organizacji.
